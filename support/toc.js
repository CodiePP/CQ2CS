...
// https://github.com/CodiePP/styleguide/blob/gh-pages/include/styleguide.js
...
function CreateTOC(tocElement) {
  ...
  tocHeading.id = 'toc';
  tocHeading.setAttribute('property', 'http://purl.org/dc/terms/tableOfContents')
...
function AddTOCMaster(tocTable, heading) {
  ...
  var lang = "all";
  if (heading.getAttribute("lang")) { lang = heading.getAttribute("lang"); }
  // Add the table row scaffolding
  var toc_tr = document.createElement('tr');
  toc_tr.setAttribute('lang', lang);
...
function LinkifyHeadersForTag(tagName) {
  ...
  for (var j = 0; j != headers.length; j++) {
    ...
    if (!hasClass(header, 'ignoreLink') && ('id' in header)) {
      if (header.id != '') {
        LinkifyHeader(header, '../support/link.png', 21, '');
    ...
    // if ('id' in header) {
    //   if (header.id != '') {
    //     LinkifyHeader(header, '../support/orig.png', 21, '');
